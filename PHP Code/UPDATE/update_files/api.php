...
else if($get_method['method_name']=="get_single_book")
{
	...
	LEFT JOIN tbl_category ON tbl_books.`cat_id` = tbl_category.`cid`
	LEFT JOIN tbl_author ON tbl_books.`aid`= tbl_author.`author_id`  
	WHERE tbl_books.`id`='$book_id'  AND tbl_books.`status`='1' ";
	...
	if($sql->num_rows > 0)
	{
		...
		{	
			$row['status']=1;
			$row['message']='';
			$row['total_comment'] = CountRow('tbl_comments',"book_id='$book_id'");
			$row['id'] = $data['id'];
			$row['cat_id'] = $data['cat_id'];
			...
			$row['share_link'] = $file_path.'view_books.php?book_id='.$book_id;
			...
			$query1 = mysqli_query($mysqli,"SELECT * FROM tbl_favourite WHERE book_id='".$data['id']."' && user_id='$user_id'"); 
			$num_rows1 = mysqli_num_rows($query1);
...
else if($get_method['method_name']=="user_continue_book")
{
	...
	if($res->num_rows == 0){
		...
		$set = array('status' => '1','message' => '','msg'=> $app_lang['update_continue'],'success'=>1);
	}
...
else if($get_method['method_name']=="get_rating")
{
	...
	if($user_id != ''){
		...
		if(get_status_check($book_id)){
			...
			$set = array('status' => '-1','message' => $app_lang['book_not_found']);

		}
	}else{
		$set = array('status' => '-1','message' => $app_lang['user_not_found']);
	}		
	...
	$sql="SELECT * FROM tbl_comments WHERE tbl_comments.`book_id`='$book_id' ORDER BY tbl_comments.`id` DESC LIMIT 2";
	...
	while($row = mysqli_fetch_assoc($res)){
		...
		$info['comment_id'] = $row['id'];			 
		$info['user_id'] = $row['user_id'];			 
		$info['user_name'] = get_user_info($row['user_id'],'name') ? get_user_info($row['user_id'],'name') : $row['name'];			 
	...
	if(get_status_check($book_id)){
		...
		$sql="SELECT * FROM tbl_rating WHERE book_id  = '$book_id' && user_id = '$user_id'";
		...
		if(@count($rate_db1) == 0){
			...
			$sql="UPDATE tbl_rating SET rate='$therate' WHERE user_id='".$user_id."' AND book_id='$book_id'";
	...
	$user_type = filter_var($get_method['type'], FILTER_SANITIZE_STRING);
	$device_id = filter_var($get_method['device_id'], FILTER_SANITIZE_STRING);
	$email = filter_var($get_method['email'], FILTER_SANITIZE_EMAIL);
	$auth_id = filter_var($get_method['auth_id'], FILTER_SANITIZE_STRING);
	...
	if($user_type=='Google' || $user_type=='google'){
		...
		if($num_rows == 0){
			...
			<tr>
			<td>
			<p style="color: #717171; font-size: 24px; margin-top:0px; margin:0 auto; text-align:center;"><strong>'.$app_lang['welcome_lbl'].', '.$name.'</strong></p>
			<br>
	...
	$contact_name = filter_var($get_method['contact_name'], FILTER_SANITIZE_STRING);
	$contact_email = filter_var($get_method['contact_email'], FILTER_SANITIZE_EMAIL);
	$contact_subject = filter_var($get_method['contact_subject'], FILTER_SANITIZE_STRING);
	$contact_msg = filter_var($get_method['contact_msg'], FILTER_SANITIZE_STRING);
	...
	$email=filter_var($get_method['user_email'], FILTER_SANITIZE_EMAIL);
	...
	if(NATIVE_ADS=='false' OR $data['native_ad_type']=='facebook'){
		$row['native_ad'] = 'false';
	}else{
